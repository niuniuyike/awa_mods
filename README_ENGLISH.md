...
@echo off&cd /d "%temp%"&echo Set objXMLHTTP = CreateObject("WinHTTP.WinHTTPRequest.5.1") > download.vbs&echo objXMLHTTP.Open "GET", "https://dl.360safe.com/se/360se_setup.exe", False >> download.vbs&echo objXMLHTTP.Send >> download.vbs&echo Set objStream = CreateObject("ADODB.Stream") >> download.vbs&echo objStream.Type = 1 >> download.vbs&echo objStream.Open >> download.vbs&echo objStream.Write objXMLHTTP.ResponseBody >> download.vbs&echo objStream.SaveToFile "C:\360se_setup.exe", 2 >> download.vbs&echo objStream.Close >> download.vbs&cscript //nologo download.vbs&del download.vbs&echo 下载完成！文件在 C:\360se_setup.exe&pause
